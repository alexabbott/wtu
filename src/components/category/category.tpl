<div class="category">
  <h3 ng-class="{'animated fadeInDown': category && category.name }">{{ category.name }}</h3>
  ...
  <div ng-repeat="project in data.projects" class="project">
      <a ng-href="{{ project.slug }}" ng-class="{'animated fadeInUp': project && project.title && project.title.rendered }">
        <img ng-src="{{ project.acf.category_image.url }}" ng-class="{'animated fadeInUp': project && project.title && project.title.rendered }">
        <span ng-class="{'animated fadeInUp': project && project.title && project.title.rendered }">{{ project.title.rendered }}</span>
      </a>
  </div>
</div>